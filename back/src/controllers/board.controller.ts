...
const getBoardsArray = async (req: Request, res: Response) => {
  try {
    const result = await BoardService.getBoardsArray();
    res.status(StatusCode.OK).json(result);
  } catch (error) {
    if (error instanceof Error) {
      res.status(StatusCode.SERVER_ERROR).json({ error: error.message });
...
export const BoardController = { createNew, getBoard, getBoardsArray, update };
